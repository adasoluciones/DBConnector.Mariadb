...
namespace Ada.Framework.Data.DBConnector.Mariadb.Mapper
{
    /// <summary>
    ...
    public sealed class QueryCreator : IQueryCreator
    {
        ...
        public string ObtenerQuery(ProcedimientoAlmacenado procedimientoAlmacenado)
        {
            ...
            ProcedimientoAlmacenado sp = procedimientoAlmacenado as ProcedimientoAlmacenado;
            retorno = string.Format("EXEC {0} ", sp.Nombre);

            bool esPrimerParametro = true;
            foreach (ParametroSql parametro in sp.Parametros.Parametros)
            {
